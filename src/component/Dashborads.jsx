...
import GradientLineChart from './GradientLineCharts';
			...
			<div className="grid lg:grid-cols-2 gap-4 mb-8">
					...
					<div className="flex justify-between items-center mb-4">
						<div>
							...
							<div className="flex gap-3 items-center">
								<h2 className="text-[24px] flex items-center ">
									{year_revenue} <FaEuroSign size={20} />
								</h2>
				...
				<div className="bg-white p-6 rounded-lg shadow-md border border-yellow-300 w-full ">
					<div className="flex justify-between items-center mb-4">
						...
						<select
							className="p-2 border rounded-md text-sm text-gray-600"
							value={selectLineBarYear}
							onChange={(e) => setSelectLineBarYear(e.target.value)}>
							{barChartData?.reverse().map((item, index) => (
								<option key={index} value={item.year}>
									{item.year}
								</option>
