...
import LogoutModal from './LogoutModals';
...
const Sidebar = ({ children }) => {
	...
	return (
		<div className="flex flex-col min-h-screen">
			...
			<aside
				id="logo-sidebar"
				className={`fixed top-0 left-0 z-40 w-[246px] h-full pt-16 transition-transform ${isSidebarOpen ? '' : '-translate-x-full'
				...
				<div className="h-full px-3 pb-4 overflow-y-auto bg-sidebar">
					<ul className="space-y-2 font-medium mt-10">
						<li>
							<Link
								to="/"
								className={`flex items-center p-2 rounded-lg group ${location.pathname === '/'
						...
						<li>
							<Link
								to="/user"
								className={`flex items-center p-2 rounded-lg group ${location.pathname === '/user'
									? 'bg-gray-200 text-gray-900'
									: 'text-gray-900 hover:bg-gray-100'
									}`}>
								<HiOutlineUsers
